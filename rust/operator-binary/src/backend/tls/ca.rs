...
impl Manager {
    ...
    pub fn trust_roots(&self, min_remaining_lifetime: Option<Duration>) -> Vec<&X509> {
        match min_remaining_lifetime {
            Some(min_lifetime) => {
                let cutoff = OffsetDateTime::now_utc() + min_lifetime;
            ...
            None => {
                // Return all CAs without any filtering
                self.certificate_authorities
                    .iter()
                    .map(|ca| &ca.certificate)
                    .chain(self.additional_trusted_certificates.iter())
                    .collect()
            }
        }
    }
