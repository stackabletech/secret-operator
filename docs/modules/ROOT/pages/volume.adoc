...
<4> The injected volume can then be mounted into a container, just like any other volume, which effectively means that the correct secrets are provided within the `/tls` directory inside of the pod
